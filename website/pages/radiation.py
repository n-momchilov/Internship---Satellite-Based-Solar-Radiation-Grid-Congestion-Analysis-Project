...
def render():
    if not UI_META_PATH.exists():
        st.warning("Precomputed radiation data not found.")
        st.info(
            "To enable this page, run **`precompute_ui_assets.ipynb`** "
            "from the `website/` folder. It generates the `.npy` and "
            "`ui_assets_meta.json` files in `data/precomputed/`."
        )
        return
